...
def stream_parser(sockets, nodes, threads, arch):
    ...
    for size in sizes:
        for mode in modes:
            route = '{}/outputs/stream/{}/{}/{}/new/'.format(cwd, arch, threads, size)
            for experiment in experiments:
                with open(route + '{}_{}.csv'.format(mode, experiment), 'w') as csvfile:
                    os.chdir(route)
                    results_writer = csv.writer(csvfile, dialect='excel')
                    if arch == 'penguin':

                        results_writer.writerow(['Sockets\Mem_Nodes', 0, 1, 2, 3])
                                ...
                                if socket1 < socket2:
                                    ...
                                    for node in range(0, int(nodes)):
                                        ...
                                        if experiment == 'membind':
                                            name = 'stream_{}_{}_{}GB_proc{}-{}_mem{}_{}t_{}*'.format(
                                                experiment, arch, size, socket1, socket2, node, threads, mode)
                                        else:
                                            name = 'stream_{}_{}_{}GB_proc{}-{}_mem0-{}_{}t_{}*'.format(
                                                experiment, arch, size, socket1, socket2, node, threads, mode)
                                        ...
                                        if name_file is not None:
                                            ...
                                            matrix_file = list()
                                            for line in file:
                                                if re.search('Copy', line) or re.search('Scale',
                                                                                        line) or re.search(
                                                        'Add', line) \
                        ...
                        for socket1 in range(0, int(sockets)):
                            for socket2 in range(0, int(sockets)):
                                for socket3 in range(0, int(sockets)):
                                    for socket4 in range(0, int(sockets)):
                                        if socket1 < socket2 < socket3 < socket4:
                                            average = [
                                                str(socket1) + '-' + str(socket2) + '-' + str(socket3) + '-' + str(socket4)]
                                            for node in range(0, int(nodes)):

                                                # stream_membind_batman_1GB_proc0_mem0_8t_sense_1936
                                                if experiment == 'membind':
                                                    name = 'stream_{}_{}_{}GB_proc{}-{}-{}-{}_mem{}_{}t_{}*'.format(
                                                        experiment, arch, size, socket1,
                                                        socket2, socket3, socket4, node, threads, mode)
                                                else:
                                                    name = 'stream_{}_{}_{}GB_proc{}-{}-{}-{}_mem0-{}_{}t_{}*'.format(
                                                        experiment, arch, size,
                                                        socket1,
                                                        socket2, socket3, socket4, node,
                                                        threads, mode)
                                                ...
                                                if name_file is not None:
                                                    ...
                                                    matrix_file = list()
                                                    for line in file:
                                                        if re.search('Copy', line) or re.search('Scale', line) or re.search(
                                                                'Add', line) \
                                                                or re.search('Triad', line):
                                                            line_exec_time = line.replace('\n', '')
                                                            line_exec_time = line_exec_time.split(' ')
